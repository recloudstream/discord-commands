    ...
    try {
      ...
      if (!repoId)
        return message.channel.send(
          "U need to add something to the command dummy, repo and plugin name"
        );
      let file = globalThis.events_src.filter(
        (json) => json.name == "repos.js"
      )[0];
      let repo = requireFromString(file.code).find(
        (it) => it?.name === repoId || it?.shortcut === repoId
      )?.raw_url;
      if (!repo && repoId.indexOf("http") === -1)
        return message.channel.send("No repo found");
      let repoResponse =
        repo &&
        (
          await axios.get(repo, {
            headers: {
      ...
      let fields = [
        ...
        },
        {
          name: "TTL",
          value: `<t:${Math.round(
            (new Date().getTime() + countResponse.ttl) / 1000
          )}:R>`,
          inline: true,
        },
      ];
        ...
        embeds: [
          {
            ...
            thumbnail: {
              url:
                plugin?.iconUrl?.replaceAll("%size%", "128") ??
                "https://cdn0.iconfinder.com/data/icons/file-management-system-flat/32/file_managemenr_system_icon_set_flat_style-14-512.png",
            },
          },
        ],
      });
