...
const issueToMeta = (issue) => {
  ...
  chunks.forEach((chunk) => {
    if (chunk === "### Addon Manifest URL") readingFor = "url";
    else if (chunk === "### Addon Description") readingFor = "description";
    else if (chunk === "### Language of Content") readingFor = "language";
    else if (chunk === "### Choose Labels") readingFor = "labels";
      ...
      if (readingFor === "labels" && chunk.toLowerCase().startsWith("- [x] ")) {
        meta.proposedLabels.push(
          chunk.replace("- [X] ", "").replace("- [x] ", "").trim()
        );
        return;
      }
      if (readingFor === "language") {
        lang = chunk.split("; ")[0].split(" (")[0].trim();
        if (lang !== "_No response_") meta[readingFor] = lang;
        return;
      }
  ...
  if (!meta.url.startsWith("https://") || !meta.url.endsWith("/manifest.json"))
    meta.url = "";
  if (meta.description === "_No response_") meta.description = "";
  ...
  async execute(message) {
    ...
    for (const label of VALID_LABELS) {
      issues.push(
        ...(
          await axios.get(
            `https://api.github.com/repos/danamag/stremio-addons-list/issues?state=open&per_page=100&labels=${label}`,
            {
              headers: {
      ...
      try {
        ...
        if (meta.url) {
          allList.push({
            ...
            value: `[${meta.name}](${meta.url.replace(
              /\/manifest\.json$/gi,
              ""
            )})`,
            inline: false,
    ...
    message.channel.send({
      content: `You can install these by using the clone site feature.`,
      embeds: allEmbeds,
    });
  },
};
