...
module.exports = {
  ...
  async execute(message) {
    var data =
      (
        await axios.get(
          "https://api.github.com/repos/recloudstream/cloudstream-archive/git/trees/master",
          {
            headers: {
    ...
    var currentAvailableCommits = data
      .filter((x) => x.path.endsWith(".apk"))
      .map((x) => x.path);
    var commits =
      (
        await axios.get(
          "https://api.github.com/repos/recloudstream/cloudstream/commits",
          {
            headers: {
              "accept-encoding": "null",
            },
    ...
    var fields = commits
      .filter((x) =>
        currentAvailableCommits.includes(`${x.sha.slice(0, 7)}.apk`)
      )
      .map((commit, idx) => {
