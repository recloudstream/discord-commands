...
module.exports = {
  ...
  async execute(message) {
    let data = (
      await axios.get(
        "https://api.github.com/repos/recloudstream/cloudstream/releases"
      )
    ).data;
    let stable = data.find(
      (it) => it && !it.prerelease && it.tag_name !== "pre-release"
    );
    let apk = stable.assets.find(
      (it) =>
        it && it.content_type === "application/vnd.android.package-archive"
    );
